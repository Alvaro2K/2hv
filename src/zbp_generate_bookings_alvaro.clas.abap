CLASS zbp_generate_bookings_alvaro DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_alvaro IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_alvaro.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_alvaro.
*   insert the new table entries
    INSERT ztbooking_alvaro FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_alvaro INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
